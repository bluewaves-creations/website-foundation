...
You can set a maximum time for how long your request can wait before it starts processing using the `X-Fal-Request-Timeout` header. This is useful when you want to fail fast if a runner isn't available within a certain time window.
...
In this example, the request will fail with a `504 Gateway Timeout` if processing doesn't start within 30 seconds.

<Note>
  **Start Timeout Behavior**

  * The timeout value is in **seconds** and must be at least 1 second
  * The timeout measures **total elapsed time from submission**, including queue wait, routing, and any retry/requeue attempts (e.g., if a request fails with 429 or 503 and is retried, all attempts count against the same timeout)
  * **Once processing starts, the timeout is no longer enforced**
  * On timeout, the response includes the header `X-Fal-Request-Timeout-Type: user` to distinguish from infrastructure timeouts
  * Requests that timed out are **not retried**—the server respects your specified timeout
</Note>
